"""Plot p_i v5 relay profiles at claim-final and answer-token sites.

Each model uses the original dark-to-light visual grammar for Active*, Macro,
and Inactive controls.  Active* is the v5 four-control pi_active_IIA aggregate.
"""
...
METRIC_STYLES = {
    "active": {"label": "Active* (4 controls)", "linewidth": 2.8},
    "macro": {"label": "Macro", "linewidth": 2.0},
    "inactive": {"label": "Inactive controls", "linewidth": 1.7},
}
PLOT_ORDER = ("inactive", "macro", "active")
SITES = ("claim_final", "answer_token")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Qwen and Phi-4 p_i v5 relay profiles across normalized depth."
    )
    parser.add_argument(
        "--qwen-csv",
        type=Path,
        default=Path("data/das/qwen_pi_v5_rawid_r16_stride2_1ep_b20/relay_map.csv"),
    )
    parser.add_argument(
        "--phi4-csv",
        type=Path,
        default=Path("data/das/phi4_pi_v5_rawid_r64_stride2_1ep_b20/relay_map.csv"),
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/das/pi_v5_normalized_depth_qwen_phi4.png"),
    )
    parser.add_argument(
        "--normalized-csv",
        type=Path,
        default=Path("data/das/pi_v5_normalized_depth_qwen_phi4.csv"),
    )
        ...
        rows.append(
            {
                ...
                "site": site,
                "active": required_float(raw, "pi_active_IIA"),
                "macro": required_float(raw, "pi_regime_macro_IIA"),
                "inactive": required_float(raw, "pi_inactive_IIA"),
                "locality": required_float(raw, "pi_locality_IIA"),
            }
        )
...
def plot(models: list[tuple[ModelSpec, list[dict]]], output: Path, dpi: int) -> None:
    ...
    fig.suptitle(r"Assumption polarity ($p_i$) relay across normalized model depth", fontsize=16)

    legend_handles = []
    for spec, _ in models:
        for metric in ("active", "macro", "inactive"):
            style = METRIC_STYLES[metric]
...
def main() -> int:
    ...
    specs = [
        ModelSpec(
            ...
            args.qwen_csv,
            args.qwen_layers,
            {"active": "#E85D04", "macro": "#F3A261", "inactive": "#FAD7B5"},
        ),
        ModelSpec(
            "Phi-4 Mini Instruct",
            args.phi4_csv,
            args.phi4_layers,
            {"active": "#1D4ED8", "macro": "#7AA7E8", "inactive": "#C7DCF7"},
        ),
    ]
